...
Event.add(defines.events.on_research_finished, function(event)
    if event and event.research then
        ...
        if (event.research.level and config.inf_res[config.mod_set][event.research.name]) and (event.research.level >= config.inf_res[config.mod_set][event.research.name]) then
