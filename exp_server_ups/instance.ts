...
export class InstancePlugin extends BaseInstancePlugin {
	...
	setInterval() {
		if (!this.updateInterval) {
			this.updateInterval = setInterval(this.updateUps.bind(this), this.instance.config.get("exp_server_ups.update_interval" as any));
		}
	}
	...
	async updateUps() {
		...
		if (collected > 0) {
			...
			ups = (maxTick - minTick) / (collected * (this.instance.config.get("exp_server_ups.update_interval" as any) / 1000));
		}
		...
		if (collected > this.instance.config.get("exp_server_ups.average_interval" as any)) {
